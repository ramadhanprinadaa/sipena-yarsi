                    ...
                    <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between text-gray-500">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <button @click="$dispatch('open-modal-add')"
                                class="flex items-center w-38 h-10 justify-center cursor-pointer bg-blue-500 hover:bg-blue-600 hover:shadow-lg text-white text-sm rounded-[10px] transition">
                                <!-- Icon -->
                        ...
                        <div class="table-wrapper">
                            <table class="table">
                                ...
                                <tbody class="divide-y divide-[#878787]/30">
                                    @forelse($cutiList as $cuti)
                                        <tr class="table-row hover:bg-[#F5F7FA]/50 transition">
                                            <td class="px-4 py-4 font-medium text-gray-700">
                                                {{ \Carbon\Carbon::parse($cuti->tanggal_pengajuan)->translatedFormat('d F Y') }}
                                            </td>
                                            ...
                                            <td class="px-4 py-4 text-center">
                                                <span
                                                    class="px-3 py-1 {{ $this->statusBadgeClass($cuti->status) }} rounded-full text-xs font-semibold">{{ $this->statusLabel($cuti->status) }}</span>
                                            </td>
                                            <td class="px-4 py-4 text-center text-gray-600">{{ $cuti->keterangan }}</td>
                                            <td class="px-4 py-4 text-center">
                                                @if ($cuti->status !== 'disetujui')
                                                    <button
                                                        wire:click="$dispatch('openModalEdit', { id: {{ $cuti->id }} })"
                                                        class="px-3 py-1.5 text-xs font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition cursor-pointer">
                                                        Edit
                                                    </button>
                                                    <button
                                                        wire:click="$dispatch('openModalDelete', { id: {{ $cuti->id }} })"
                                                        class="px-3 py-1.5 text-xs font-medium text-white bg-red-600 rounded-lg hover:bg-red-700 transition cursor-pointer">
                                                        Hapus
                                                    </button>
                                                @else
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="9" class="px-4 py-8 text-center text-gray-500">No data
                                                available</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                ...
                <div x-show="activeTab === 'rekapitulasi'" class="flex flex-col space-y-4 h-full min-h-0">
                    ...
                    <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                        ...
                        <div class="flex flex-col gap-3 md:flex-row md:items-center">
                            <div class="flex flex-wrap gap-3">
                                <input type="date" wire:model.live="filterRekapStartDate"
                                    class="w-48 h-10 px-3 text-sm bg-white border-2 border-indigo-200 rounded-[10px] focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition cursor-pointer hover:border-indigo-400">
                            ...
                            <div class="relative w-48">
                                <div
                                    class="w-48 h-10 px-3 text-sm bg-indigo-50 border-2 border-indigo-100 rounded-[10px] flex items-center text-indigo-600 font-medium">
                        ...
                        <button wire:click="exportRekapExcel"
                            class="flex items-center w-38 h-10 justify-center cursor-pointer bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 hover:shadow-xl text-white text-sm font-semibold rounded-[10px] transition duration-300 ease-in-out transform hover:scale-105">
                            <!-- Icon -->
                            <i class="fa-solid fa-download mr-2"></i> Export Excel
                        </button>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div
                            class="group relative bg-gradient-to-br from-indigo-500 via-indigo-600 to-blue-600 rounded-[16px] p-5 shadow-lg hover:shadow-2xl transition duration-300 transform hover:-translate-y-1 overflow-hidden border border-white/20">
                            <div class="relative z-10 flex items-center justify-between">
                                <div>
                                    ...
                                    <h4 class="text-3xl font-black text-white mt-1">
                                        {{ $rekapSummary['cuti_terpakai_total'] }}</h4>
                        ...
                        <div
                            class="group relative bg-gradient-to-br from-emerald-500 via-teal-600 to-cyan-600 rounded-[16px] p-5 shadow-lg hover:shadow-2xl transition duration-300 transform hover:-translate-y-1 overflow-hidden border border-white/20">
                            <div class="relative z-10 flex items-center justify-between">
                                <div>
                                    <p class="text-xs font-semibold text-white/80 uppercase">Sisa Saldo</p>
                                    <h4 class="text-3xl font-black text-white mt-1">{{ $rekapSummary['sisa_saldo'] }}
                                    </h4>
                        ...
                        <div
                            class="group relative bg-gradient-to-br from-amber-400 via-orange-500 to-red-500 rounded-[16px] p-5 shadow-lg hover:shadow-2xl transition duration-300 transform hover:-translate-y-1 overflow-hidden border border-white/20">
                            <div class="relative z-10 flex items-center justify-between">
                                <div>
                                    <p class="text-xs font-semibold text-white/80 uppercase">Cuti Tahunan</p>
                                    <h4 class="text-3xl font-black text-white mt-1">
                                        {{ $rekapSummary['cuti_terpakai_tahunan'] }}</h4>
                        ...
                        <div
                            class="group relative bg-gradient-to-br from-purple-500 via-pink-600 to-rose-600 rounded-[16px] p-5 shadow-lg hover:shadow-2xl transition duration-300 transform hover:-translate-y-1 overflow-hidden border border-white/20">
                            <div class="relative z-10 flex items-center justify-between">
                                <div>
                                    <p class="text-xs font-semibold text-white/80 uppercase">Besar/Melahirkan</p>
                                    <h4 class="text-3xl font-black text-white mt-1">
                                        {{ $rekapSummary['cuti_terpakai_besar'] + $rekapSummary['cuti_terpakai_melahirkan'] }}
                                    </h4>
                    ...
                    <div class="flex-1 min-h-0 overflow-y-auto no-scrollbar space-y-3">
                        @forelse($rekapList as $cuti)
                            <div
                                class="relative group bg-gradient-to-r from-white via-indigo-50/30 to-white border-2 border-indigo-100 hover:border-indigo-400 rounded-[16px] p-5 shadow-md hover:shadow-xl transition duration-300 overflow-hidden">
                                <div
                                    class="absolute inset-0 bg-gradient-to-r from-indigo-500/0 via-indigo-500/5 to-indigo-500/0 opacity-0 group-hover:opacity-100 transition duration-300 pointer-events-none">
                                </div>
                                <div
                                    class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                    <div class="flex items-start gap-4">
                                        <div
                                            class="bg-gradient-to-br from-indigo-500 to-indigo-700 rounded-2xl p-3.5 text-white flex-shrink-0 shadow-lg group-hover:scale-110 transition duration-300">
                                            <i
                                                class="fa-solid {{ $cuti->jenisCuti?->dihitung_per_jam ? 'fa-clock' : 'fa-plane-departure' }} text-xl"></i>
                                        </div>
                                        <div>
                                            <div class="flex items-center gap-2">
                                                <span
                                                    class="text-lg font-bold text-gray-800">{{ \Carbon\Carbon::parse($cuti->tanggal_mulai)->translatedFormat('d M') }}
                                                    -
                                                    {{ \Carbon\Carbon::parse($cuti->tanggal_selesai)->translatedFormat('d M Y') }}</span>
                                                <span
                                                    class="px-2 py-0.5 bg-indigo-100 text-indigo-700 text-[10px] font-black uppercase tracking-wider rounded-md">{{ $cuti->jenisCuti->nama ?? '-' }}</span>
                                            </div>
                                            <div
                                                class="mt-1 flex items-center gap-4 text-xs font-medium text-gray-500">
                                                <span><i
                                                        class="fa-solid fa-file-signature mr-1.5 text-indigo-400"></i>Diajukan:
                                                    {{ \Carbon\Carbon::parse($cuti->tanggal_pengajuan)->translatedFormat('d M Y') }}</span>
                                                <span><i
                                                        class="fa-solid fa-chart-line mr-1.5 text-emerald-400"></i>Sisa
                                                    Saldo: {{ $cuti->saldo_cuti_sesudah ?? '-' }} Hari</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span
                                            class="px-4 py-2 bg-emerald-100 text-emerald-700 rounded-full text-xs font-bold shadow-sm border border-emerald-200">
                                ...
                                <div class="mt-4 pl-16">
                                    <div
                                        class="bg-gray-50 rounded-xl p-3 border border-gray-100 group-hover:bg-white transition duration-300">
                                        <p class="text-sm text-gray-600 italic"><i
                                                class="fa-solid fa-quote-left mr-2 text-indigo-300"></i>{{ $cuti->keterangan }}
                                        </p>
                                    </div>
                                </div>
                            </div>
                        @empty
                            <div
                                class="bg-gray-50 border-2 border-dashed border-gray-300 rounded-[16px] p-12 text-center">
                                <div
                                ...
                                <p class="text-gray-500 font-bold">Tidak ada data rekapitulasi cuti dalam periode ini
                                </p>
                                <p class="text-gray-400 text-xs mt-1">Coba ubah filter tanggal untuk melihat data
                                    lainnya</p>
                            </div>
                        @endforelse
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="bg-gradient-to-br from-blue-400 via-blue-500 to-indigo-600 rounded-[20px] p-6 shadow-lg">
                        ...
                        <div class="border-t border-white/20 pt-4 mt-4">
                            <p class="text-blue-100 text-xs">Berakhir pada <span class="font-semibold">31 Desember
                                    {{ now()->year }}</span></p>
                        </div>
                    </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <div>
                                <div class="flex justify-between items-center mb-1">
                                    ...
                                    <span
                                        class="text-white text-xs font-bold">{{ $rekapSummary['cuti_terpakai_tahunan'] }}/12
                                        Hari</span>
                                </div>
                                <div class="w-full bg-white/30 rounded-full h-2">
                                    <div class="bg-blue-400 h-2 rounded-full"
                                        style="width: {{ $rekapSummary['cuti_terpakai_tahunan'] > 0 ? min(100, ($rekapSummary['cuti_terpakai_tahunan'] / 12) * 100) : 0 }}%">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                <div class="flex justify-between items-center mb-1">
                                    ...
                                    <span
                                        class="text-white text-xs font-bold">{{ $rekapSummary['cuti_terpakai_besar'] }}/66
                                        Hari</span>
                                </div>
                                <div class="w-full bg-white/30 rounded-full h-2">
                                    <div class="bg-red-400 h-2 rounded-full"
                                        style="width: {{ $rekapSummary['cuti_terpakai_besar'] > 0 ? min(100, ($rekapSummary['cuti_terpakai_besar'] / 66) * 100) : 0 }}%">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                <div class="flex justify-between items-center mb-1">
                                    ...
                                    <span
                                        class="text-white text-xs font-bold">{{ $rekapSummary['cuti_terpakai_melahirkan'] }}/90
                                        Hari</span>
                                </div>
                                <div class="w-full bg-white/30 rounded-full h-2">
                                    <div class="bg-pink-400 h-2 rounded-full"
                                        style="width: {{ $rekapSummary['cuti_terpakai_melahirkan'] > 0 ? min(100, ($rekapSummary['cuti_terpakai_melahirkan'] / 90) * 100) : 0 }}%">
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="flex items-start justify-between mb-4">
                            <div>
                                ...
                                <h3 class="text-4xl font-bold text-white">
                                    {{ $rekapSummary['cuti_terpakai'] + $rekapSummary['cuti_terpakai_besar'] + $rekapSummary['cuti_terpakai_melahirkan'] }}
                                </h3>
