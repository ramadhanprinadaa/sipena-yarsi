<div class="flex flex-col space-y-6 min-h-[calc(100vh-380px)]">
    <!-- Header -->
    ...
    <div class="flex-1 relative rounded-md shadow-md border border-slate-100 border-t-4 border-t-slate-500 p-4 flex">
        @if (!$this->rekening)
            <div class="flex flex-col items-center text-center max-w-sm mx-auto py-12">
                <div
                    class="w-14 h-14 bg-slate-100 text-slate-600 rounded-full flex items-center justify-center mb-4 border border-dashed border-slate-200">
                    <i class="fa-solid fa-credit-card text-xl"></i>
                </div>
                <h4 class="text-base font-semibold text-slate-700">Data Rekening Belum Tersedia</h4>
                    ...
                    <div
                        ...
                        <div class="mt-8">
                            ...
                            <h4 class="text-xl font-semibold tracking-widest font-mono mt-0.5 text-slate-100">
                                {{ chunk_split($this->rekening->nomor_rekening, 4, ' ') }}
                            </h4>
                        </div>
                ...
                <div class="lg:col-span-5 border-t lg:border-t-0 lg:border-l border-slate-100 lg:pl-8 space-y-3">
                    <div>
                        ...
                        <dl class="grid grid-cols-3 gap-y-2 text-xs leading-relaxed">
                            ...
                            <dd class="text-slate-700 col-span-2">: {{ $this->rekening->nama_rekening }}</dd>
                        </dl>
                    </div>
                    ...
                    <div class="bg-slate-50 rounded-lg p-4 border border-slate-100 flex items-start gap-3.5">
                        ...
                        <div class="space-y-0.5">
                            ...
                            <p class="text-xs font-medium text-indigo-600">
                                {{ $this->rekening->editor_name ?? 'Sistem / Migrasi Awal' }}
                            </p>
                            <p class="text-[10px] text-slate-400">
                                Pada tanggal:
                                {{ \Carbon\Carbon::parse($this->rekening->updated_at)->format('d M Y, H:i') }} WIB
                            </p>
                        </div>
                    </div>
