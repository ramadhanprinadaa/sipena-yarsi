    ...
    <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between text-gray-500">
        ...
        <div class="flex flex-wrap gap-3">
            ...
            <div class="relative w-48">
                <select wire:model.live="selectedRole"
                    class="w-48 h-10 px-2 text-sm bg-white border border-gray-200 rounded-md focus:outline-none focus:ring-4 focus:ring-indigo-500 cursor-pointer appearance-none">
        ...
        <div class="flex gap-3">
            ...
            <div class="relative w-72">
                ...
                <input type="text" class="input-search h-10" wire:model.live.debounce.300ms="search"
                    placeholder="Cari Nama atau NIP ...">
            </div>

            <!-- Add User Button -->
            <button @click="$dispatch('open-add-modal')"
                class="flex items-center px-3 justify-center cursor-pointer bg-indigo-500 hover:bg-indigo-700 text-white text-sm rounded-md transition">
        ...
        <div class="table-wrapper">
            <table class="table">
                ...
                <thead class="table-header">
                    <tr>
                        <th wire:click="sortBy('username')" scope="col" class="px-4 py-3 font-medium w-30">
                ...
                <tbody>
                    @foreach ($users as $user)
                        <tr wire:key="user-{{ $user->id }}" class="table-row">
                            ...
                            <!-- Nama -->
                            <td class="px-4 py-2 font-medium truncate" title="{{ $user->pegawai->nama ?? 'N/A' }}">
                                {{ $user->pegawai->nama ?? '-' }}
                            </td>
                            ...
                            <td class="px-4 py-2">
                                @if ($user->status == 'active')
                                    <span
                                        class="px-2.5 py-1 text-xs font-medium rounded-md bg-green-100 text-green-700">
                                        Aktif
                                    </span>
                                @else
        ...
        <div class="text-body bg-neutral-secondary-medium border-t border-default-medium rounded-md">
            ...
            <nav class="flex items-center flex-column flex-wrap md:flex-row justify-between px-4 py-2"
                ...
                <span class="text-sm font-normal text-body block w-full md:inline md:w-auto">
                    Menampilkan
                    <span class="font-semibold text-heading">{{ $users->firstItem() }}-{{ $users->lastItem() }}</span>
                    dari
                ...
                <ul class="flex -space-x-px text-sm border border-gray-300 rounded-lg">
                    <li>
                        <button wire:click="gotoPage(1)" @disabled($users->onFirstPage())
                    ...
                    <button wire:click="gotoPage({{ $users->lastPage() }})" @disabled($users->onLastPage())
