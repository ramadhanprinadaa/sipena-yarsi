<div class="relative bg-white min-w-2xl min-h-[75vh] mx-auto rounded-2xl shadow-2xl flex flex-col overflow-hidden">
    <!-- Header -->
    <header
        class="flex items-center justify-between px-5 py-4 border-b border-slate-200 shrink-0 bg-gradient-to-r from-amber-50 via-white to-yellow-50">

        <!-- Decoration -->
        <div class="absolute inset-0 opacity-40 pointer-events-none">
            <div class="absolute -top-10 -right-10 w-40 h-40 bg-amber-200 rounded-full blur-3xl"></div>
            <div class="absolute -bottom-10 -left-10 w-40 h-40 bg-yellow-200 rounded-full blur-3xl"></div>
        </div>

        <!-- Header Information -->
        <div class="relative flex items-center gap-4">
            <div
                class="w-11 h-11 rounded-2xl bg-amber-500 flex items-center justify-center shadow-lg">
            ...
            <div>
                ...
                <p class="text-xs text-gray-400 mt-0.5">
                    Perbarui data riwayat pendidikan pegawai {{ $nama_pegawai }}
                </p>
            </div>
        </div>
        ...
        <div class="relative flex items-center gap-2">
            <button type="button" @click="$dispatch('close-edit-pendidikan-modal')" wire:loading.attr="disabled"
        ...
        <div class="grid grid-cols-2 gap-x-6 gap-y-4">
            ...
            <div
                ...
                <button type="button" x-on:click="show = !show"
                    class="w-full flex items-center justify-between h-10 px-1 border-b-2 border-gray-300 hover:border-amber-400 focus-within:border-amber-500 text-sm transition cursor-pointer">
            ...
            <div class="grid grid-cols-2 gap-4 col-span-2">
                <div>
                    <label for="edit_tahun_masuk" class="block text-xs font-medium text-gray-600 mb-1">
                ...
                <div>
                    <label for="edit_tahun_lulus" class="block text-xs font-medium text-gray-600 mb-1">
            ...
            <div class="col-span-2">
                <label for="edit_file_ijazah" class="block text-xs font-medium text-gray-600 mb-1">
                    File Ijazah
                </label>

                <!-- Info file yang sudah tersimpan sebelumnya -->
                @if($existingFileName)
                    <div class="flex items-center gap-2 mb-2 px-3 py-2 bg-slate-50 border border-slate-200 rounded-md">
                        <i class="fa-solid fa-file-lines text-slate-400"></i>
                        <span class="text-xs text-slate-600 truncate">{{ $existingFileName }}</span>
                    </div>
                @endif

                <input type="file" id="edit_file_ijazah" wire:model.live.blur="form.file_ijazah" accept=".pdf,.doc,.docx"
                    class="w-full border-2 rounded-md shadow-none focus:ring-0 border-gray-300 focus:border-amber-500 focus:outline-none text-sm placeholder:italic placeholder-gray-400">
                <p class="mt-1 text-xs text-gray-400">
                    Kosongkan jika tidak ingin mengubah file. Format PDF, DOC, atau DOCX. Maksimal 10 MB.
                </p>
    ...
    <!-- Footer -->
    <div class="flex gap-2 p-4 border-t border-gray-200 justify-between">

        <div class="flex items-center gap-1.5 text-xs text-amber-600 {{ $this->isDirty ? '' : 'invisible' }}">
            <i class="fa-solid fa-circle-exclamation"></i>
            <span>Terdapat perubahan yang belum disimpan.</span>
        </div>

        <div class="flex justify-end gap-3">
            <button type="button" @click="$dispatch('close-edit-pendidikan-modal')"
            ...
            <button type="button"
                ...
                @disabled(
                    blank($form['jenjang_pendidikan_id']) ||
                    blank($form['tahun_masuk']) ||
                    blank($form['tahun_lulus']) ||
                    $errors->any() ||
                    !$this->isDirty)
