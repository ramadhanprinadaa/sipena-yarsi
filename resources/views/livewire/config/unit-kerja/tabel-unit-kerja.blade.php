<div class="flex flex-col space-y-3 h-[calc(100vh-280px)]">
    ...
    <div class="flex items-center justify-between gap-2">
        ...
        <div class="relative w-87">
            ...
            <input type="text" class="input-search text-sm" wire:model.live.debounce.300ms="search"
                placeholder="Cari Nama Unit atau Pimpinan ...">
        </div>
        ...
        <div class="flex gap-2">
            ...
            <div class="relative w-46" x-data="{ open: false, selected: 'Semua Unit SDM' }">
                <button @click="open = !open" wire:model.live="selectedUnitSdm" class="filter-dropdown" type="button">
                ...
                <div x-show="open" @click.outside="open = false" x-transition class="dropdown-menu">
                    <ul class="p-2 text-sm text-body font-medium">
                        <li>
                            <button @click="selected='Semua Unit SDM'; open=false" class="dropdown-item"
                                wire:click="$set('selectedUnitSdm', '')">
                                Semua Unit SDM
                            </button>
                        </li>
                        <li>
                            <button @click="selected='SDM Yayasan'; open=false" class="dropdown-item"
                                wire:click="$set('selectedUnitSdm', '1')">
                                SDM Yayasan
                            </button>
                        </li>
                        <li>
                            <button @click="selected='SDM Universitas'; open=false" class="dropdown-item"
                                wire:click="$set('selectedUnitSdm', '2')">
                                SDM Universitas
                            </button>
                        </li>
                    </ul>
                        ...
                        <tr class="table-row">
                            ...
                            <td class="px-4 py-2 truncate">
                                <div class="flex items-center gap-2">
                                    @if ($unit->pimpinan_id != null)
                                        <div
                                            class="w-8 h-8 rounded-full bg-amber-100 flex items-center justify-center flex-shrink-0">
                                            @php
                            ...
                            <td class="px-4 py-2 text-center">
                                @if ($unit->unit_sdm_id == 1)
                                    <span
                                        class="px-2.5 py-1 text-xs font-medium bg-violet-100 text-violet-600 rounded-md">
                                        SDM Yayasan
                                    </span>
                                @else
                                    <span
                                        class="px-2.5 py-1 text-xs font-medium bg-emerald-100 text-emerald-700 rounded-md">
                                        SDM Universitas
                                    </span>
                                @endif
                            </td>
                            <td class="px-4 py-2 text-center">
                                <button @click="$dispatch('open-loading-detail')"
        ...
        <div class="text-body bg-neutral-secondary-medium border-t border-default-medium rounded-md">
            <nav class="flex items-center flex-column flex-wrap md:flex-row justify-between px-4 py-2"
                ...
                <span class="text-sm font-normal text-body block w-full md:inline md:w-auto">
                    Menampilkan
                    <span
                        class="font-semibold text-heading">{{ $unit_kerja->firstItem() }}-{{ $unit_kerja->lastItem() }}</span>
                    dari
                ...
                <ul class="flex -space-x-px text-sm border border-gray-300 rounded-lg">
                    <li>
                        <button wire:click="gotoPage(1)" @disabled($unit_kerja->onFirstPage())
                            class="table-pagination-btn rounded-s-lg px-3 cursor-pointer disabled:cursor-not-allowed disabled:opacity-50">
                            <i class="fa-solid fa-angles-left text-xs"></i>
                        </button>
                    </li>
                    <li>
                        <button wire:click="previousPage" @disabled($unit_kerja->onFirstPage())
                            class="table-pagination-btn px-3 cursor-pointer disabled:cursor-not-allowed disabled:opacity-50">
                            Previous
                        </button>
                    </li>
                    @for ($i = max(1, $unit_kerja->currentPage() - 3); $i <= min($unit_kerja->lastPage(), $unit_kerja->currentPage() + 3); $i++)
                        <li>
                    ...
                    @endfor
                    <li>
                        <button wire:click="nextPage" @disabled(!$unit_kerja->hasMorePages())
                            class="table-pagination-btn px-3 cursor-pointer disabled:cursor-not-allowed disabled:opacity-50">
                            Next
                        </button>
                    </li>
                    <button wire:click="gotoPage({{ $unit_kerja->lastPage() }})" @disabled($unit_kerja->onLastPage())
